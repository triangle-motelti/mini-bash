...
/*   Updated: 2025/06/26 21:13:43 by motelti          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
//         return (free(key), free(value), 1);
//     }
//     if (is_append && (!value || value[0] == '\0'))  // Modified this condition
//     {
//         ft_putstr_fd("minishell: export: `", 2);
//         ft_putstr_fd(arg, 2);
//         ft_putstr_fd("': not a valid identifier\n", 2);
...
int validate_export_key(char *arg, char *key, char *value, int is_append)
{
    ...
    // Check for empty append (+= with no value)
    if (is_append && value && value[0] == '\0')
    {
        ft_putstr_fd("minishell: export: `", 2);
        ft_putstr_fd(arg, 2);
...
int non_append_case(t_shell *mini, t_env *found, char *key, char *value)
{
    ...
    if (found)
    {
        ret = update_env_node(found, value);  // UNCOMMENT THIS LINE
        free(key);
    }
    else
        ret = append_env_node(mini, key, value);
